...
async function init(){
  if(!SUPABASE_URL || !SUPABASE_ANON_KEY){
    app.innerHTML = `<div class="login-wrap"><div class="login-card"><div class="kicker">Missing .env</div><h1>Supabase keys missing</h1><p class="muted">Create a .env file in this admin folder using .env.example.</p></div></div>`;
    return;
  }
...
function render(){ session ? renderAdmin() : renderLogin(); }
  ...
  app.innerHTML = `
    <div class="login-wrap">
      <form class="login-card" id="loginForm">
        ...
        <div class="form-group"><label>Email</label><input class="input" id="email" type="email" required placeholder="reimagebs@gmail.com"></div>
        <div class="form-group"><label>Password</label><input class="input" id="password" type="password" required placeholder="Password"></div>
...
async function handleLogin(e){
  ...
  notice.className='notice show'; notice.textContent='Signing in...';
  const { error } = await supabase.auth.signInWithPassword({
    email: document.getElementById('email').value.trim(),
    password: document.getElementById('password').value
  });
  if(error){ notice.className='notice show error'; notice.textContent=error.message; }
}
...
async function renderAdmin(){
  app.innerHTML = `
    <div class="admin-shell">
      <header class="topbar">
        <div class="brand"><img src="/logo.png" alt="RE IMAGE logo" onerror="this.style.display='none'"><span>Admin Portal</span></div>
        <div class="top-actions"><span class="admin-email">${escapeHtml(session.user.email||'')}</span><button class="btn btn-light" id="refreshBtn">Refresh</button><button class="btn btn-light" id="signOutBtn">Sign Out</button></div>
      </header>
      <main class="main">
        <div id="stats"></div>
        <div class="tabs"><button class="tab ${activeView==='list'?'active':''}" data-view="list">List View</button><button class="tab ${activeView==='calendar'?'active':''}" data-view="calendar">Calendar View</button></div>
        <div class="toolbar"><input class="input" id="searchInput" placeholder="Search name, email, business, message..." value="${escapeAttr(filters.search)}"><select id="serviceFilter"></select><select id="statusFilter"></select><button class="btn btn-primary" id="applyFiltersBtn">Apply</button></div>
...
function bindTopEvents(){
  ...
  document.querySelectorAll('.tab').forEach(btn=>btn.addEventListener('click',()=>{activeView=btn.dataset.view;renderAdmin();}));
  document.getElementById('applyFiltersBtn').addEventListener('click',()=>{filters.search=document.getElementById('searchInput').value.trim();filters.service=document.getElementById('serviceFilter').value;filters.status=document.getElementById('statusFilter').value;renderContent();});
}

async function loadRequests(){
  const area=document.getElementById('contentArea');
  if(area) area.innerHTML='<div class="table-card"><div class="detail-body muted">Loading submissions...</div></div>';
  const { data,error }=await supabase.from('start_requests').select('*').order('created_at',{ascending:false});
  if(error){area.innerHTML=`<div class="table-card"><div class="detail-body notice show error">${escapeHtml(error.message)}</div></div>`;return;}
  requests=data||[]; if(!selectedId && requests.length) selectedId=requests[0].id;
  renderStats(); populateFilters(); renderContent();
}

function populateFilters(){
  const service=document.getElementById('serviceFilter'), status=document.getElementById('statusFilter');
  if(service) service.innerHTML=serviceOptions();
  if(status) status.innerHTML=statusOptions();
}
function renderStats(){
  document.getElementById('stats').innerHTML=`<section class="stats"><div class="stat-card"><span>Total Submissions</span><strong>${requests.length}</strong></div><div class="stat-card"><span>New Leads</span><strong>${requests.filter(r=>(r.status||'new')==='new').length}</strong></div><div class="stat-card"><span>Consultations</span><strong>${requests.filter(r=>r.service_choice==='Consultation').length}</strong></div><div class="stat-card"><span>Booked Slots</span><strong>${requests.filter(r=>r.consultation_date&&r.consultation_time).length}</strong></div></section>`;
}
function renderContent(){activeView==='calendar'?renderCalendarView():renderListView();}
function filteredRequests(){
  const s=filters.search.toLowerCase();
  return requests.filter(r=>{
    const blob=`${r.first_name||''} ${r.last_name||''} ${r.email||''} ${r.phone||''} ${r.business_name||''} ${r.message||''}`.toLowerCase();
    return (!s||blob.includes(s))&&(filters.service==='all'||r.service_choice===filters.service)&&(filters.status==='all'||(r.status||'new')===filters.status);
  });
}
...
function renderCalendarView(){
  ...
  const currentMonthLabel = calViewDate.toLocaleDateString('en-US', { month:'long', year:'numeric' });
...
function renderCrmMonthCalendar(consults){
  ...
  const grouped = consults.reduce((acc, r) => {
    if (!acc[r.consultation_date]) acc[r.consultation_date] = [];
  ...
  document.querySelectorAll('.crm-event').forEach(eventBtn => {
    eventBtn.addEventListener('click', (e) => {
      ...
      if (request) openCalendarRequestModal(request);
    });
  });
}
...
function handleModalEscape(e){
  if(e.key === 'Escape') closeCalendarRequestModal();
}
...
function statusOptions(){return ['all',...STATUS_OPTIONS].map(s=>`<option value="${s}" ${filters.status===s?'selected':''}>${s==='all'?'All Statuses':titleCase(s)}</option>`).join('');}
function statusBadge(status='new'){const s=status||'new';return`<span class="badge badge-${escapeAttr(s)}">${escapeHtml(titleCase(s))}</span>`;}
function consultationLabel(r){return r&&r.consultation_date?`${formatDateOnly(r.consultation_date)} ${r.consultation_time||''}`:'—';}
function parseLocalDate(iso){const[y,m,d]=String(iso).split('-').map(Number);return new Date(y,m-1,d);}
function formatDateOnly(iso){return iso?parseLocalDate(iso).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}):'—';}
function formatDateTime(value){return value?new Date(value).toLocaleString('en-US',{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit'}):'—';}
function fullName(r){return`${r.first_name||''} ${r.last_name||''}`.trim()||'Unknown';}
